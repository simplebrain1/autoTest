...
class TestHomeSearchClickBootTime(BaseTestCase):
    def setup(self):
        self.tag = '打开首页搜索'
        print("you are testing apk is :" + self.current_pkg + " ,apk_cls:" + self.current_cls + ",tag:" + self.tag)
        self.save_file_name = "TestHomeSearchClickBootTime"
    ...
    def test_action(self):
        print("TestHomeSearchClickBootTime is start")
        duration = self.home.cold_boot_time_home_view_click(self.home.home_search_click_view,
                                                            self.home.home_search_show_view)
...
if __name__ == "__main__":
    d = u2.connect()
    test = TestHomeSearchClickBootTime(d)  # 创建测试对象
    test.setup()  # 初始化
    test.test_case()  # 开始测试
