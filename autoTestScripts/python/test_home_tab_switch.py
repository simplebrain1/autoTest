...
"""
主页切Tab帧信息TestCase
执行完脚本，自动操作并在目前下/FpsFiles/homeTab/下生成对应的帧数据文件fps_temp.txt
"""


class TestHomeTabSwitch(BaseTestCase):
    def setup(self):
        self.tag = "主页切Tab帧信息测试"
        print("you are testing apk is :" + self.current_pkg + " ,apk_cls:" + self.current_cls + ",tag:" + self.tag)
        self.save_file_name = "homeTab"
    ...
    def run(self, total):
        if self.version > 23:
            ...
            cmd = "sh /data/local/tmp/fps_info.sh {0} {1} {2} {3}".format(total,
                                                                          ...
                                                                          self.home.target_cls + "/" + self.home.target_cls + "#0",
                                                                          "1")
    ...
    def test_action(self):
        ...
        try:
            ...
            self.d.sleep(1)
            print("testHomeTabSwitch is end")
        except Exception as e:
            self.home.watcher.stop()
            self.test_completion.set()
            self._end_test()
            print("testHomeTabSwitch is exception!".format(e))
            raise e


if __name__ == "__main__":
    d = u2.connect()
    test = TestHomeTabSwitch(d)  # 创建测试对象
    test.setup()  # 初始化
    test.test_case()  # 开始测试
