...
def get_range(_train_or_forecast, _forecast_file_name, _param, _picture_path):
    ...
    if _train_or_forecast == 'train':
        ...
        # 把分好类的稳定阶段的图片存本地
        try:
            shutil.rmtree(_picture_path + '/train_stable_' + file_name)
            shutil.rmtree(_picture_path + '/train_unstable_' + file_name)
        except OSError as e:
            print("Error: %s : %s" % ('/train_stable_', e.strerror))
        ...
        try:
            shutil.rmtree(_picture_path + '/forecast_stable_' + file_name)
            shutil.rmtree(_picture_path + '/forecast_unstable_' + file_name)
        except OSError as e:
